...
/**
 * @brief Fungsi untuk mencari buku berdasarkan judulnya dalam Circular Linked
 * List.
...
bool cariBuku(BukuNode *head, string judulDicari) {
  // membuat var sementara agar tidak terjadi perubahan pada head
  BukuNode *temp = head;
  // melakukan perulangan
  while (true) {
    // cek jika judul sama makan return true
    if (temp->judulBuku == judulDicari) {
      return true;
    }
    // cek  jik judul tidak sama dan node selanjutnya bukan head maka pindah
    else if (temp->next != head) {
      temp = temp->next;
    }
    // jika else if diatas false berarti sudah berada di head dan return false
    else {
      return false;
    }
  }
...
int main() {
  ...
  cout << "Mencari buku '" << buku1
       << "': " << (cariBuku(head, buku1) ? "Ditemukan" : "Tidak Ditemukan")
       << endl;
  // Harusnya output: Ditemukan

  cout << "Mencari buku '" << buku2
       << "': " << (cariBuku(head, buku2) ? "Ditemukan" : "Tidak Ditemukan")
       << endl;
